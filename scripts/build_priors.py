...
def main():
    ...
    # Load alignment
    ids, seqs = utils.load_alignment(args.input)

    # Build window profiles
    profiles, seqs_filtered, ref_seq = utils.build_window_profiles(
        ids, seqs, args.ref, args.win, args.overlap
    )

    # Compute priors
    df = utils.build_priors(seqs_filtered, profiles)

    # Save to Parquet
    utils.save_priors(df, args.output)
